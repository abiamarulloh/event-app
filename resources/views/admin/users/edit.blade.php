<x-app-layout>
    <div class="sm:ml-64">
        ...
        <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8 rounded-lg my-4">
            <h2 class="text-2xl font-semibold mb-4">Ubah user</h2>
            @if ($errors->any())
            ...
            <form action="{{ route('user.update', $user->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" value="{{ old('name', $user->name ?  $user->name : '') }}" name="name" id="title"
                        class="w-full p-2.5 border border-gray-600 rounded focus:ring-blue-500 focus:border-blue-500">
                </div>

                <div class="mb-4">
                    <label for="email" class="block text-sm font-medium mb-2">Email</label>
                    <input type="text" value="{{ old('email', $user->email ?  $user->email : '') }}" name="email" id="email"
                        class="w-full p-2.5 border border-gray-600 rounded focus:ring-blue-500 focus:border-blue-500">
                </div>
